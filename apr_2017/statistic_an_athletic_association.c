 ...
 * the individual results of a team of 5:
 *
 ...
 *
 * Mean or Average : To calculate mean, add together all of 
 * the numbers in a set and then divide the sum by the total count of numbers.
 *
 ...
 * middle value; the median is then defined to be the mean of the two middle
 * values (the median of {3, 5, 6, 9} is (5 + 6) / 2 = 5.5).
 *
...
char* stat(char* strg) {
    ...
    printf ("\n Total number of entry is = %d\n", count+1);
    h = malloc (sizeof (int) * (count+1));
    m = malloc (sizeof (int) * (count+1));
    s = malloc (sizeof (int) * (count+1));
...
int main () {
    char* l = "01|15|59, 01|47|16, 01|17|20, 01|32|34, 02|17|17";
    stat (l);
    printf ("\n");
}
